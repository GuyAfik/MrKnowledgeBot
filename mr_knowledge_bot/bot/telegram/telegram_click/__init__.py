...
from mr_knowledge_bot.bot.telegram.telegram_click.util import \
    escape_for_markdown
...
def generate_command_list(update: Update, context: CallbackContext, summary: bool = False) -> str:
    ...
    if summary:
        return '\n'.join(
            [
                '/' + "|".join([escape_for_markdown(command_name)
                               for command_name in command.get('names')])
        ...
        sorted_commands = sorted(commands_not_hidden, key=lambda x: (
            x[KEY_NAMES][0].lower(), len(x[KEY_ARGUMENTS])))
        help_messages = list(
            map(lambda x: x[KEY_HELP_MESSAGE], sorted_commands))
